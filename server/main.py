...
def scrape_prok_notices_job():
    """스크래퍼 백그라운드 작업: 총회 홈페이지 공지사항 최근 5개를 가져와 local SQLite DB에 저장 및 동기화"""
    import urllib.request
    ...
    logging.info("[Scraper] Starting PROK notice scraper job...")
    ...
    try:
        html, _ = fetch_html(index_url)
        soup = BeautifulSoup(html, 'html.parser')
        table = soup.find('table', class_='table-hover')
        if not table:
            logging.error("[Scraper] Notice list table not found.")
            return
        ...
        if not parsed_posts:
            logging.warning("[Scraper] No posts parsed from board index.")
            return
        ...
        for p in top_5:
            ...
            try:
                ...
                if row:
                    ...
                    logging.info(f"[Scraper] Updated notice {notice_id} (ID: {post_id})")
                else:
                    c.execute("""
                    ...
                    logging.info(f"[Scraper] Inserted new notice (ID: {post_id})")
            except Exception as detail_err:
                logging.error(f"[Scraper] Error fetching detail for post {post_id}: {detail_err}")

        # Delete older scraped notices
        c.execute("""
            DELETE FROM notices 
            WHERE scope = 'assembly' 
        ...
        if deleted_count > 0:
            logging.info(f"[Scraper] Cleaned up {deleted_count} old scraped notices.")

        conn.commit()
        conn.close()
        logging.info("[Scraper] Scraper job completed successfully.")
    except Exception as e:
        logging.error(f"[Scraper] Scraper job failed with error: {e}")
...
async def start_scheduler():
    ...
    # Schedule PROK board scraper to run every 2 hours
    scheduler.add_job(scrape_prok_notices_job, 'interval', hours=2)
    scheduler.start()
    logging.info("[Scheduler] Started AsyncIOScheduler. replicate_mssql_to_local_scheduled is scheduled for Mondays at 04:00 AM.")
    logging.info("[Scheduler] Scheduled scrape_prok_notices_job for every 2 hours.")
    
    # Run once immediately in a background thread
    import threading
    threading.Thread(target=scrape_prok_notices_job, daemon=True).start()
